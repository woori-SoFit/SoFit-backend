...
1. **Redis Hash 구조 사용**: 인증코드(authCode)와 함께 bankCode, accountNumber를 하나의 Redis Hash에 저장하여 TTL 기반 만료를 일괄 관리한다.
2. **Rate Limiter Redis 기반 구현**: 계좌번호당 일일 요청 횟수를 Redis INCR + TTL(자정까지 남은 시간)로 관리하여 별도 스케줄러 없이 자동 초기화한다.
3. **RestClient 사용**: Spring Boot 4.x 환경에서 코데프 API 호출을 위해 RestClient를 사용한다.
4. **임시 인증 컨텍스트**: 세션 미구현 상태에서 하드코딩된 userId/applicationId를 사용하며, 추후 세션 기반으로 전환 가능하도록 설계한다.
...
sequenceDiagram
    ...
    participant Controller as AccountVerificationController
    ...
    Client->>Controller: POST /api/account-verification
    Controller->>Validator: 입력값 검증 (bankCode, accountNumber)
    Validator-->>Controller: 검증 결과
    Controller->>Service: requestTransfer(request)
    ...
    Service-->>Controller: TransferResponse (bankName, maskedAccount, expiredAt)
    Controller-->>Client: ApiResponse<TransferResponse>

    Note over Client, DB: Phase 2: 인증코드 확인
    Client->>Controller: POST /api/account-verification/confirm
    Controller->>Validator: 입력값 검증 (verificationCode)
    Controller->>Service: confirmVerification(request)
    ...
    Service->>DB: INSERT/UPDATE loan_execution (bank_code, account_number)
    Service-->>Controller: ConfirmResponse (accountVerified: true)
    Controller-->>Client: ApiResponse<ConfirmResponse>
```
...
│   └── AccountVerificationControllerDocs.java
├── dto/
│   ├── request/
│   │   ├── AccountVerificationRequest.java
│   │   └── AccountVerificationConfirmRequest.java
│   └── response/
│       ├── AccountVerificationResponse.java
│       └── AccountVerificationConfirmResponse.java
├── service/
│   ├── AccountVerificationService.java
│   └── AccountVerificationServiceImpl.java
├── client/
│   └── CodefClient.java
├── util/
│   └── AccountMaskingUtil.java
└── exception/
    └── (LoanErrorCode에 추가)
```
...
### 1. AccountVerificationController
...
public class AccountVerificationController implements AccountVerificationControllerDocs {

    // POST /api/account-verification - 1원 송금 요청
    public ApiResponse<AccountVerificationResponse> requestVerification(
        @Valid @RequestBody AccountVerificationRequest request);

    // POST /api/account-verification/confirm - 인증코드 확인
    public ApiResponse<AccountVerificationConfirmResponse> confirmVerification(
        @Valid @RequestBody AccountVerificationConfirmRequest request);
}
```

### 2. AccountVerificationService

계좌 인증 비즈니스 로직을 처리하는 서비스 인터페이스.

```java
public interface AccountVerificationService {
    AccountVerificationResponse requestTransfer(AccountVerificationRequest request);
    AccountVerificationConfirmResponse confirmVerification(AccountVerificationConfirmRequest request);
}
```
...
public class AccountVerificationRequest {
    ...
    private Long applicationId;  // nullable, 기본값 1L
}
...
| AccountVerificationServiceImpl | 비즈니스 로직 (Mock: CodefClient, Redis, Repository) |
