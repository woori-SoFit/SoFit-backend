...
- [x] 1. Phase 1: 인프라/설정 (환경변수, RestClient, SecurityConfig)
  - [x] 1.1 application.yml에 External Mock URL 환경변수 추가 및 .env.example 업데이트
    - `sofit-user/src/main/resources/application.yml`에 `external.mock.url` 설정 추가
    - `.env.example`에 `EXTERNAL_MOCK_URL` 항목 추가
    - _Requirements: 3.1_

  - [x] 1.2 ExternalMockConfig 생성 (RestClient Bean 등록)
  ...
  - [x] 1.3 SecurityConfig 생성 (permitAll, 세션 정책)
...
- [x] 2. Phase 2: Entity 확장 (BusinessProfile 필드 추가, Enum)
  - [x] 2.1 BusinessProfileStatus enum 생성 (sofit-common)
    - `sofit-common/src/main/java/com/sofit/common/entity/auth/BusinessProfileStatus.java` 생성
    - PENDING, VERIFIED, FAILED 값 정의
    - common에 위치하여 멀티모듈에서 공유 가능
    - _Requirements: 1.3_

  - [x] 2.2 BusinessProfile 엔티티 확장 (sofit-common)
    - `sofit-common/src/main/java/com/sofit/common/entity/auth/BusinessProfile.java` 수정
    ...
    - `createVerified(User, ...)` 정적 팩토리 메서드 추가
    - _Requirements: 1.3, 1.7_

- [x] 3. Phase 3: Exception/Code 정의
  - [x] 3.1 AuthErrorCode enum 생성
