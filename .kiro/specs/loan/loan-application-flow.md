...
```
POST /api/loans/products/{productId}/eligibility-check
```
...
- **eligible = true 이면** → `loan_application` 레코드를 **DRAFT 상태로 즉시 DB에 생성**
- 응답으로 `applicationId` 반환 → 이후 모든 단계는 이 ID 기반으로 진행
