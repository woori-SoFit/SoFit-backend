package com.sofit.admin.global.filter;
 ...
 * - 서버 사이: user↔admin 호출 관계가 없으므로 전파 대신 DB 추적 컬럼(request_id)으로 흔적을 남긴다.
...
public class TraceIdFilter extends OncePerRequestFilter {
    ...
    private String resolveClientIp(HttpServletRequest request) {
        ...
        if (forwarded != null && !forwarded.isBlank()) {
            return forwarded.split(",")[0].trim();   // 프록시/Nginx 체인의 최초 클라이언트
        }
